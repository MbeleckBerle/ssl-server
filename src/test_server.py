...
def test_string_not_found_reread():
    with tempfile.NamedTemporaryFile(delete=False, mode='w') as tmp_file:
...
def test_string_exists_cached(reset_cache):
    with tempfile.NamedTemporaryFile(delete=False, mode='w') as tmp_file:
        tmp_file.write("cached_line\nanother_line")
        tmp_file_path = tmp_file.name
    try:
        result = search_string_in_file_cached(tmp_file_path, "cached_line")
        assert result == "STRING EXISTS"
    finally:
        os.remove(tmp_file_path)
...
def test_log_search_write_error():
...
def search_file():
    with tempfile.NamedTemporaryFile(delete=False, mode='w') as tmp_file:
        ...
        search_file_path = tmp_file.name
    yield search_file_path
    os.remove(search_file_path)

@pytest.fixture(autouse=True)
def setup_handle_client(search_file):
    import server
    server.FILE_PATH = search_file
    server.REREAD_ON_QUERY = True

def test_handle_client_valid_query(search_file):
    fake_conn = MagicMock()
    fake_addr = "127.0.0.1"
...
def test_handle_client_empty_query(search_file):
    fake_conn = MagicMock()
    fake_addr = "127.0.0.1"
...
class Test200kData:
    ...
    @pytest.fixture(autouse=True)
    def data_file(self):
        data_file = os.path.join(os.path.dirname(__file__), "200k.txt")
        yield data_file
        assert os.path.exists(data_file)

    def test_file_exists(self, data_file):
        assert os.path.exists(data_file)

    def test_content_length(self, data_file):
