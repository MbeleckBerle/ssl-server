...
def load_config(config_path: str = "config.ini") -> Tuple[
    ...
    The config file:
      ...
      - REREAD_ON_QUERY: reread the file on every query (True/False).
      - SSL_ENABLED: SSL is enabled for secure communications (True/False).
    ...
    try:
        ...
        reread_on_query: bool = (
            config["DEFAULT"].get("REREAD_ON_QUERY",
                                  "False").strip().lower() == "true"
        )
        ssl_enabled: bool = (
            config["DEFAULT"].get("SSL_ENABLED",
                                  "False").strip().lower() == "true"
        )
        ...
        if not os.path.exists(path):
            print(
                f"ERROR: Configured file '{path}' does not exist. "
        ...
        if ssl_enabled:
            if not os.path.exists(certfile) or not os.path.exists(keyfile):
                print(
                    f"ERROR: SSL is enabled but certificate \
                        '{certfile}' or key "
        ...
        print(
            f"ERROR: Missing key {e} in {config_path}. \
...
def search_string_in_file(path: str, query: str, reread_on_query: bool) -> str:
    ...
    :param reread_on_query: Flag to indicate reread the file on every query.
    ...
    try:
        if reread_on_query:
            ...
            return search_string_in_file_cached(path, query)
...
def search_string_in_file_cached(path: str, query: str) -> str:
    """
...
def handle_client(conn: socket.socket, addr: Tuple[str, int]) -> None:
    ...
    while True:
        try:
            # Receive data with overflow protection
            data: bytes = conn.recv(BUFFER_SIZE)

            if len(data) > BUFFER_SIZE:
                print(f"Warning: Received data exceeds buffer size limit from {addr}. Ignoring potential overflow attempt.")
                continue

            # Ensure that received data is in proper format
            ...
            if data == "":
                ...
                exe_time: float = round((time.time() - start_time) * 1000, 3)
