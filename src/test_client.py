...
def mock_socket_and_ssl() -> Generator[Tuple[MagicMock, MagicMock], None, None]:
    ...
    with patch('socket.socket', return_value=fake_socket), patch('ssl.create_default_context', return_value=fake_ssl_context):
...
def mock_config() -> Generator[None, None, None]:
    ...
    with patch("os.path.exists", return_value=True), \
         ...
         patch("configparser.ConfigParser.get", side_effect=lambda section, option, **kwargs: "True" if option == "SSL_ENABLED" else "server_cert.pem"):
        yield
